...
router.post('/link-teamlead', async (req, res) => {
    try {
        const aseId = req.user.userId;
        const { teamLeadId } = req.body;
        if (!teamLeadId) {
            res.status(400).json({ error: 'teamLeadId required' });
            return;
        }
        const tl = await prisma_1.default.teamLead.findUnique({ where: { id: teamLeadId } });
        if (!tl) {
            res.status(404).json({ error: 'Team Lead not found' });
            return;
        }
        if (tl.aseId && tl.aseId !== aseId) {
            res.status(409).json({ error: 'This TL is already linked to another ASE' });
            return;
        }
        const updated = await prisma_1.default.teamLead.update({
            where: { id: teamLeadId },
            data: { aseId },
        });
        res.json(updated);
...
router.delete('/link-teamlead/:id', async (req, res) => {
    try {
        const aseId = req.user.userId;
        const tl = await prisma_1.default.teamLead.findUnique({ where: { id: req.params.id } });
        if (!tl || tl.aseId !== aseId) {
            res.status(403).json({ error: 'Not authorised' });
            return;
        }
        await prisma_1.default.teamLead.update({ where: { id: req.params.id }, data: { aseId: null } });
        res.json({ ok: true });
    }
    catch {
...
// ── PATCH /api/v1/ase/teamleads/:id — edit TL details ──────────────────────
router.patch('/teamleads/:id', async (req, res) => {
    try {
        const { userId } = req.user;
        const { id } = req.params;
        // Verify TL belongs to this ASE
        const tl = await prisma_1.default.teamLead.findFirst({ where: { id, aseId: userId }, include: { user: true } });
        if (!tl) {
            res.status(404).json({ success: false, error: 'Team lead not found' });
            return;
        }
...
// ── DELETE /api/v1/ase/teamleads/:id — unlink TL from ASE ──────────────────
router.delete('/teamleads/:id', async (req, res) => {
    try {
        const { userId } = req.user;
        const { id } = req.params;
        const tl = await prisma_1.default.teamLead.findFirst({ where: { id, aseId: userId } });
        if (!tl) {
            res.status(404).json({ success: false, error: 'Team lead not found' });
            return;
        }
        // Unlink: set aseId to null (preserve all history)
        await prisma_1.default.teamLead.update({ where: { id }, data: { aseId: null } });
        res.json({ success: true, data: { message: 'Team lead unlinked from your team' } });
    }
    catch (err) {
        console.error(err);
        res.status(500).json({ success: false, error: 'Server error' });
...
// ── GET /api/v1/ase/teamleads/:id/performance — yesterday/weekly/monthly ───
...
router.get('/export', async (req, res) => {
    try {
        ...
        const aseUser = await prisma_1.default.user.findUnique({ where: { id: userId } });
        if (!aseUser) {
            res.status(404).json({ success: false, error: 'ASE not found' });
            return;
        }
        const teamLeads = await prisma_1.default.teamLead.findMany({
            where: { aseId: userId },
        ...
        for (const tl of teamLeads) {
            const allDsas = await prisma_1.default.dSA.findMany({ where: { teamLeadId: tl.id }, include: { activations: { where: { date: { gte: mtdStart, lte: today } } } } });
