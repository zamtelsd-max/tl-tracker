...
router.get('/export', async (req, res) => {
    try {
        ...
        for (const dsa of tl.dsas) {
            const actsAgg = await prisma_1.default.activation.aggregate({
                where: { dsaId: dsa.id, date: { gte: mtdStart, lte: today } },
                _sum: { count: true },
            });
            const gasCount = await prisma_1.default.grossAdd.count({
                where: { dsaId: dsa.id, date: { gte: mtdStart, lte: today } },
            });
            sheet1.addRow({
        ...
        const todayActs = await prisma_1.default.activation.findMany({
        ...
        const gasRows = await prisma_1.default.grossAdd.findMany({
